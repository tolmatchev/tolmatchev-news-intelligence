...
public class RssService {
    ...
    public RssService(RssClient rssClient, RestClient restClient, @Value("${rss.url}") String rssUrl,
    ...
    public void saveNewNews(TassRssResponse data) {
        ...
        Set<String> existingLinks = newsRepository.findExistingLinks(data.channel().items().stream().map(TassItemDto::link).toList());
