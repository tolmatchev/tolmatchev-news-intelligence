...
class RssClientTest {
  ...
  @MockitoBean private NewsRepository newsRepository;
  ...
  @Autowired private RssClient rssClient;

  @Test
  void shouldFetchAndParseRssFeed() throws Exception {
    String xml =
        new ClassPathResource("rss/tass-rss.xml").getContentAsString(StandardCharsets.UTF_8);

    wireMock.stubFor(
        get(urlEqualTo("/rss/v2.xml"))
            .willReturn(
                aResponse().withHeader("Content-Type", "application/rss+xml").withBody(xml)));

    assertThat(rssClient.fetchTassRss()).isEqualTo(expected());
  }

  private static TassRssResponse expected() {
    return new TassRssResponse(
        new TassChannelDto(
            List.of(
                new TassItemDto(
